...
- **盘中先手（不踏空上板 / 大阳）**：先手靠**盘前挂单**、不靠盘前判断——收盘后买区已定，次日开盘前把**风险预算法算出的股数**挂在买区上沿，**有效期 5 个交易日**（T+1 成交 63.4%，肯等到 T+5 升到 79.8%）。盘中在**三条通道**上开口：**A 量能突变**（≥10:00、当根 5 分钟量 ≥ 当日此前最大 2.5 倍、启动前振幅 ≤2.5%、触发价 ≤ 开盘+1.0×ATR）；**B 关键位突破**（盘前定 K，5 分钟收盘 > K+0.10×ATR，上限 = K + D_max×ATR，**D_max：A 股 0.60 / 美股 1.20**）；**C 分钟回踩确认**（A 或 B **曾放行**即可 —— 不要求当前新鲜，C 专为「错过第一根」而设；**相对前一根下跌**的 5 分钟根回踩启动点 ±0.30×ATR 不破 + **未爆量**（< max(启动根量, 当日此前均量)×2.0）+ 收阳且低点抬高企稳；止损贴回踩低点 −0.10×ATR，**风险最小**）。优先级 **C > A > B**。**仓位用风险预算法（两轨同式）**：`min(账户 × 1.5% / (入场 − 硬止损), 账户 × 上限 / 入场)`，向下取整到申报单位（A 股 100/200 股、美股 1 股）；上限 A 股 30%（T+1 隔夜跌停 −3% 倒推）/ 美股 50%（旧口径「计划仓 1/3」已废弃 —— 与风险无关）。**双轨分歧点只有「当天能不能纠错」**：A 股 T+1 → 只能小追、止损是**次日**口径、**距涨停 <1.5% 不买**；美股 T+0 → 可多追一档、破位即走。**两轨都不设收盘时间限制** —— 挂单 / 回踩只看价不看钟（旧的「14:30 后不开新仓」已否掉，它会让点位到了却买不到）。**买入上限闸门**：挂单/追单不得高于 `P_max = (T1 + 1.5×hard) / 2.5`，否则盈亏比跌破 1.5:1。**T+1 高开处置**：≤+1% 原样挂 ｜ +1~3% 只挂不追 ｜ >+3% 回踩单作废。主模式作废 / `wait` 时给出**次级观察位**（回踩前低收盘不破 + 缩量）。硬红线：开盘 30 分钟内不试、涨停/一字板不追、现价出买区上沿不市价买、当日已止损不试。脚本 `probe_intraday.py`。
...
2. **A 股 T+1 是结构问题，不是参数问题。** 所有通道 T+1 净账户全负（−0.09% ~ −0.18%）。
   把止损放宽到 0.8×ATR 只把累计 R 从 −38.9 改善到 −18.2，净账户几乎不动 —— 风险预算法
   会按新止损等比缩股，R 的改善是尺度效应。
3. **一批「未决参数」其实是非约束参数**：`BREAK_DMAX_ASH`（0.40~1.00 结果几乎相同）、
   `BO_NEAR_STRONG`（0.60~1.20 四档结果一字不差，因 121 条样本里 `strong` 一次未触发）、
   `BO_BODY_ATR` / `BO_VOL_REL` / `PULLBACK_VOL_MULT` / `PRE_AMP_MAX` 同理。
   通道 A 更极端：903 个标的日只触发 **3 次**。继续调这些数字没有边际收益。
