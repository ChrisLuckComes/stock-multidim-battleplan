...
```
输出四块。**A 股**：①预案单（挂单价=买区上沿、**风险预算法股数**、结构+硬止损、撤单线、**买入上限 `P_max`**、**T+1 高开处置**）；①·B **突破预案单**（只给「等回踩」是残缺的——强势票贴着前高收时它永不成交。故按「基准日收盘距上方 K 的距离」分三档：≤0.80×ATR 且实体 ≥1.0×ATR、量 ≥1.5×前20日均量 → **strong**；≤0.50×ATR → **normal**；>0.80×ATR → **far 只报位置、不给挂单价**。给出「站上 K+0.10×ATR 买入 / 止损 K−0.30×ATR（T+1 次日口径）/ 目标 = 近 60 根内最近阻力或 K+2.0×ATR」，附开盘撤单线 —— 跳空 ≥ 触发+0.60×ATR 或跌破 K−1.0×ATR 即撤；距涨停 <1.5% 放弃）；②盘中量能突变五档（时间窗 / 量能突变 / 蓄势位置 / 未追高 / **离涨停 ≥1.5%**）；②·B **关键位突破**（盘前定 K，触发 K+0.10×ATR，上限 **K+0.60×ATR**，**T+1 → 止损是次日口径**，触发价距涨停 <1.5% 即弃）；②·C **回踩确认**（需 ② 或 ②·B 曾放行；入场 = 企稳根收盘，止损 = 回踩低 −0.10×ATR，另受「距涨停 ≥1.5%」限制，**不受收盘时间限制**）；③次级观察位（主模式作废或 wait 时，回踩前低收盘不破 + 缩量为小仓试错区）。

**美股**：①预案单（同构，但股数按**风险预算法** = `账户 × 1.5% / (入场 − 硬止损)`，再用 `账户 × 50%` 夹上限，1 股起）；①·B **突破预案单**（同 A 股逻辑、上限用美股档 `K+1.20×ATR`，无涨停约束。SDGR 9/15 实证：K=21.40、触发 21.50、止损 21.09、116 股、最大亏 $48 → 当日收 23.25，**+$203 = 4.27R**）；②**盘前通道**（北京 16:00–21:30，美股独有——盘前价在买区内则挂上沿，高于买入上限则撤单）；③盘中量能突变（北京 21:30–04:00，跨日自动归属交易日，蓄势门槛按 `max(2.5%, 1.0×ATR/H)` 归一化）；④**关键位突破**（上限 **K+1.20×ATR**——T+0 破位即走，可比 A 股多追一档）；④·C **回踩确认**（需 ③ 或 ④ 曾放行；入场 = 企稳根收盘、止损 = 回踩低 −0.10×ATR；ILMN 9/15 实测：风险 1.43/股 vs 通道 B 的 4.34/股，**降低 67%**）。
