...
- **盘中先手（不踏空上板 / 大阳）**：先手靠**盘前挂单**、不靠盘前判断——收盘后买区已定，次日开盘前把 1/3 仓挂在买区上沿，**有效期 5 个交易日**（T+1 成交 63.4%，肯等到 T+5 升到 79.8%）。盘中在**三条通道**上开口：**A 量能突变**（≥10:00、当根 5 分钟量 ≥ 当日此前最大 2.5 倍、启动前振幅 ≤2.5%、触发价 ≤ 开盘+1.0×ATR）；**B 关键位突破**（盘前定 K，5 分钟收盘 > K+0.10×ATR，上限 = K + D_max×ATR，**D_max：A 股 0.60 / 美股 1.20**）；**C 分钟回踩确认**（A 或 B **曾放行**即可 —— 不要求当前新鲜，C 专为「错过第一根」而设；**相对前一根下跌**的 5 分钟根回踩启动点 ±0.30×ATR 不破 + **未爆量**（< max(启动根量, 当日此前均量)×2.0）+ 收阳且低点抬高企稳；止损贴回踩低点 −0.10×ATR，**风险最小**）。优先级 **C > A > B**，仓位 ≤ 账户 30%（按 T+1 隔夜跌停 −3% 倒推）。**双轨分歧点只有「当天能不能纠错」**：A 股 T+1 → 只能小追、止损是**次日**口径、**距涨停 <1.5% 不买**、14:30 后不开新仓；美股 T+0 → 可多追一档、破位即走。**买入上限闸门**：挂单/追单不得高于 `P_max = (T1 + 1.5×hard) / 2.5`，否则盈亏比跌破 1.5:1。**T+1 高开处置**：≤+1% 原样挂 ｜ +1~3% 只挂不追 ｜ >+3% 回踩单作废。主模式作废 / `wait` 时给出**次级观察位**（回踩前低收盘不破 + 缩量）。硬红线：开盘 30 分钟内不试、涨停/一字板不追、现价出买区上沿不市价买、当日已止损不试。脚本 `probe_intraday.py`。
...
├── probe_intraday.py    # 盘中先手判定（预案单 / 盘中四条通道 / 次级观察位）
...
```
输出四块。**A 股**：①预案单（挂单价=买区上沿、1/3 股数、结构+硬止损、撤单线、**买入上限 `P_max`**、**T+1 高开处置**）；②盘中量能突变五档（时间窗 / 量能突变 / 蓄势位置 / 未追高 / **离涨停 ≥1.5%**）；②·B **关键位突破**（盘前定 K，触发 K+0.10×ATR，上限 **K+0.60×ATR**，**T+1 → 止损是次日口径**，触发价距涨停 <1.5% 即弃）；②·C **回踩确认**（需 ② 或 ②·B 曾放行；入场 = 企稳根收盘，止损 = 回踩低 −0.10×ATR，另受「距涨停 ≥1.5%」与 **14:30 前**限制）；③次级观察位（主模式作废或 wait 时，回踩前低收盘不破 + 缩量为小仓试错区）。

**美股**：①预案单（同构，但股数按**风险预算法** = `账户 × 1.5% / (入场 − 硬止损)`，再用 `账户 × 50%` 夹上限，1 股起）；②**盘前通道**（北京 16:00–21:30，美股独有——盘前价在买区内则挂上沿，高于买入上限则撤单）；③盘中量能突变（北京 21:30–04:00，跨日自动归属交易日，蓄势门槛按 `max(2.5%, 1.0×ATR/H)` 归一化）；④**关键位突破**（上限 **K+1.20×ATR**——T+0 破位即走，可比 A 股多追一档）；④·C **回踩确认**（需 ③ 或 ④ 曾放行；入场 = 企稳根收盘、止损 = 回踩低 −0.10×ATR；ILMN 9/15 实测：风险 1.43/股 vs 通道 B 的 4.34/股，**降低 67%**）。
