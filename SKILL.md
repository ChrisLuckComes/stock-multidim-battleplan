...
- 每只票必须扫大额解禁、减持、立案调查及重大利空。
- 双轨打分固定为投资价值与短线博弈各 0–10 分，禁止合成总分。
- 用户点名个股时必须找 1–3 只同板块龙头并运行同一结构判定，开仓优先龙头。
...
3. **对龙头执行本 skill**：至少对每只入选龙头取数一次并跑 `rule123`，并给模式卡、扫雷、双轨打分、简版计划（可略短于主标的，但不得省略 mode / 止损要点 / 双轨分）。WorkBuddy 通达信连接器可用时，主标的与全部龙头的独立查询必须并行发出；脚本路径统一先 `fetch_market --out`，再 `rule123 --data`，禁止 `rule123` 再次联网取同一票。主标的与龙头对比写进同一份对话结论；龙头可另落 HTML，或在主报告里设「龙头对照」专节。
4. **开仓优先级**：  
