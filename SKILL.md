...
11. **买点选择前先做「股性体检」**（`python stock_character.py <code>`）：两个维度 —— **① 突破后行为**、**② 利好兑现习惯**。判据只决定「用哪种买法 / 能不能在公告日动手」，**不决定「买不买」**（与第 9 条情绪闸门同一分工：软约束只换做法，不否决）。
    ...
    **② 利好兑现习惯**（2026-09-24 追加）—— 自动抓该股历史公告、筛出**利好类事件**（BD 授权 / 合作 / 中标 / 业绩预增 / 获批 / 回购增持…），定位**真实反应日**（公告日已显著异动则算当日，否则算次日 —— 覆盖盘后发公告），统计**「反应日收盘买入」后 5 日的期望收益**。
    ...
    ⚠️ **两个维度互相独立**：博济 300404 / 东材科技 601208 / 有研硅 688432 同为「突破加速型」，利好习惯却分别是有效 / 即顶 / 有效 ⇒ **第二层给的是第一层给不出的信息**。成本：首次约 2 秒（联网抓公告），当日缓存后 < 1 秒；`--no-ann` 离线跳过，`--events YYYYMMDD,...` 手动指定利好日。见 [trade-lessons.md](references/trade-lessons.md) L002。
