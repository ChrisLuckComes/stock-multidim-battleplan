...
11. **买点选择前先做「股性体检」**（`python stock_character.py <code>`）：**同一个「突破」信号，在不同标的上的期望值可以相反** —— 关键是这只票**突破后能不能连续涨**，还是**拉一根大阳就要调几天**。判据：取历史单日涨幅 ≥5% 的大阳，以「大阳当日收盘」为买点（= 突破追入），统计其后 5 日高低点，**赔率 = 后 5 日最高价均值 ÷ |后 5 日最低价均值|**。
    ...
    实证：睿创微纳 688002 近一年 20 根大阳，赔率 **0.87**、冲击率（后 5 日摸到 −3%）**80%** ⇒ 消化型，突破当日追入是负期望（该股是 2026-09 **最大亏损源 −1,528.71**）。反例方向：博济医药 300404 / 诺诚健华 688428 判为加速型、振华股份 603067 判为消化型。**这条判据决定「用哪种买法」，不决定「买不买」**（与第 9 条情绪闸门的分工相同：软约束只换做法，不否决）。见 [trade-lessons.md](references/trade-lessons.md) L002。
