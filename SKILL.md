...
11. **买点选择前先做「股性体检」**（`python stock_character.py <code>`）：两个维度 —— **① 突破后行为**、**② 利好兑现习惯**。判据只决定「用哪种买法 / 能不能在公告日动手」，**不决定「买不买」**（与第 9 条情绪闸门同一分工：软约束只换做法，不否决）。
    **① 突破后行为** —— **同一个「突破」信号，在不同标的上的期望值可以相反** —— 关键是这只票**突破后能不能连续涨**，还是**拉一根大阳就要调几天**。判据：取历史单日涨幅 ≥5% 的大阳，以「大阳当日收盘」为买点（= 突破追入），统计其后 5 日高低点，**赔率 = 后 5 日最高价均值 ÷ |后 5 日最低价均值|**。
    ...
    实证：睿创微纳 688002 近一年 20 根大阳，赔率 **0.87**、冲击率（后 5 日摸到 −3%）**80%** ⇒ 消化型，突破当日追入是负期望（该股是 2026-09 **最大亏损源 −1,528.71**）。反例方向：博济医药 300404 / 诺诚健华 688428 判为加速型、振华股份 603067 判为消化型。
...
12. **T0 先判「刚收复」还是「沿线上行」**（`rule123.ma_ride_state()`，2026-09-24 定）：**不看「今天距某条均线几个点」，看「哪条线已经向上走了多久」**。MA5、EMA10、MA20 各自算：20 根斜率 > 0 且近 20 根 ≥12 根收在线上。有多条成立时，取收盘下方、离现价最近的那条，回踩就锚这条，不写死五日线。三条都不成立、且 MA5 斜率 ≤ 0 且站上根数 ≤11 ⇒ `fresh_reclaim`（均线刚收复，T0 原定场景）；其余 `mixed`。
    - `line_ride` 态**不接管 mode**：入口改道为 `line_pullback` —— **回踩被选中的那条均线挂限价**（买区 线−0.05×ATR ~ 线+1.0×ATR，结构止损=收盘破该线，硬止损=线−0.10×ATR），过昨高降为次选（若仍走须把硬止损锚换成「阳线下沿 / 大阳中点」）。记为 `t0_held_for_ride`（带 `line/line_label/anchor`），报告徽标写「T0 已改道」。⚠️ 字段口径：`line*` = 被选中的那条线，`ma5*` / `above20` 恒为 MA5，报告与池表**不得混用**（全池 42 只 `line_ride` 里 21 只锚非 MA5，取 ma5 会报错价位）。
    - 结果挂在 `plan["ma_ride"]`，**与 T0 是否成立解耦**（东材 601208 09-24 创 25 根新高 ⇒ T0 不成立，但它就是 `line_ride`）；报告中作为独立徽标「模式判别 = …」始终展示。
    - **这是改道不是否决**：买区在现价下方时是可预挂的限价单，不违反「有 setup 就该能挂单入场」。
    - 作废的旧口径：`D0 收盘距 MA5 ≤ 4%` —— 那是 4 只票 × 6 个月拟合的；53 只 × 500 根全池按**真实触发价 + MA5 硬止损**回放，方向直接反转（>4% 均R −0.032 vs ≤4% −0.196），机制是**离得近 ⇒ MA5 锚只有 2.3% ⇒ 77% 被毛刺扫掉**。实测表与稳健性见 [strategy-modes.md](references/strategy-modes.md) 第 ④ 条；回归 `python test_ma_ride.py`（21 项）。
