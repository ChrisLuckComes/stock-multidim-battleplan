...
12. **T0 先判「刚收复」还是「沿线上行」；且沿线只是背景、当日形态是事件**（`rule123.ma_ride_state()`，2026-09-24 三轮定稿）：**不看「今天距某条均线几个点」，看「哪条线已经向上走了多久、够不够得着」**。MA5、EMA10、MA20 各自算：20 根斜率 > 0 且近 20 根 ≥12 根收在线上。四态：
    ...
    - `new_high` —— 有线满足上行条件、**但一条都不在可回踩距离内** ⇒ **新高·无回踩锚**，`prefer=breakout`，**不给回踩买点**（老罗原话：「没有锚点就代表是新高，不需要找锚」）。门槛 1.5×ATR 的落点：距线 0.5~1.0×ATR 时 5 日内回踩触及率 66%、触及后期望 +1.59%；>1.5×ATR 时触及率跌到 **24%**、期望转负 **−0.22%** —— 这条线已不是可用回踩锚（振华 603067 09-24 锚 MA20 距 **2.20×ATR** 即此类）。
    ...
    - **这是改道不是否决**：买区在现价下方时是可预挂的限价单，不违反「有 setup 就该能挂单入场」。改道时记为 `t0_held_for_ride`（带 `line/line_label/anchor`），报告徽标写「T0 已改道」；未改道时写「T0 未接管：当日 X 是事件、沿线只是背景 → 突破优先」。⚠️ 字段口径：`line*` = 被选中的那条线，`ma5*` / `above20` 恒为 MA5，报告与池表**不得混用**（全池 42 只 `line_ride` 里 21 只锚非 MA5，取 ma5 会报错价位）。
    - 结果挂在 `plan["ma_ride"]`，**与 T0 是否成立解耦**（东材 601208 09-24 创 25 根新高 ⇒ T0 不成立，但它就是 `line_ride`）；报告中作为独立徽标「模式判别 = …」始终展示。
    - 作废的旧口径：`D0 收盘距 MA5 ≤ 4%` —— 那是 4 只票 × 6 个月拟合的；53 只 × 500 根全池按**真实触发价 + MA5 硬止损**回放，方向直接反转（>4% 均R −0.032 vs ≤4% −0.196）。⚠️ 真正的问题是**止损锚太紧**（离得近 ⇒ MA5 锚只有 2.3% ⇒ 77% 被毛刺扫掉），不是「追高」本身。实测表与稳健性见 [strategy-modes.md](references/strategy-modes.md) 第 ④ 条；回归 `python test_ma_ride.py`（28 项），复现 `python research_ride_priority.py`。
