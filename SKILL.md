...
12. **T0 先判「刚收复」还是「沿线上行」**（`rule123.ma_ride_state()`，2026-09-24 定）：**不看「今天距 MA5 几个点」，看「这条线已经向上走了多久」**。判据（自归一化、无价格阈值）：MA5 **20 根斜率 > 0 且近 20 根 ≥12 根收在线上** ⇒ `line_ride`（沿五日线上升，趋势中段）；**斜率 ≤ 0 且 ≤11 根** ⇒ `fresh_reclaim`（均线刚收复，T0 原定场景）；其余 `mixed`。
    - `line_ride` 态**不接管 mode**：入口改道为 `line_pullback` —— **回踩 MA5 挂限价**（买区 MA5−0.05×ATR ~ MA5+1.0×ATR），过昨高降为次选（若仍走须把硬止损锚换成「阳线下沿 / 大阳中点」）。记为 `t0_held_for_ride`，报告徽标写「T0 已改道」。
    ...
    - 作废的旧口径：`D0 收盘距 MA5 ≤ 4%` —— 那是 4 只票 × 6 个月拟合的；53 只 × 500 根全池按**真实触发价 + MA5 硬止损**回放，方向直接反转（>4% 均R −0.032 vs ≤4% −0.196），机制是**离得近 ⇒ MA5 锚只有 2.3% ⇒ 77% 被毛刺扫掉**。实测表与稳健性见 [strategy-modes.md](references/strategy-modes.md) 第 ④ 条；回归 `python test_ma_ride.py`（13 项）。
