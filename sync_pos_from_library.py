...
# ★ 2026-09-26 修 bug：原先写成 `os.path.join(dirname(__file__), "..")`，
#   把 HERE 算成了「skills/ 父目录」，导致本地池路径落到
#   `skills/watch/watch_cn.json`（不存在），且 `import account_config` 失败
#   （account_config 在技能根目录）。HERE 应是「技能根目录」本身。
...
def _resolve_pool_path(pool_name: str, explicit: str | None = None) -> str:
    """定位本地池 json。

    2026-09-26 修复前：HERE 被多算一层 `..`、且路径硬拼 `watch/` 子目录，
    在 INST 布局下落到 `skills/watch/watch_cn.json`（不存在）。
    实际本地池在**技能根目录** `stock-multidim-battleplan/watch_cn.json`
    （文件名带 watch_ 前缀，但不在 watch/ 子目录）——与 battle_analyze / rule123
    读取的是同一文件，写回路一致才不会「隐性落后」。
    按候选顺序返回首个存在的路径；都不存在则回退首选（技能根），
    让后续「找不到本地池」报错指向正确位置。
    """
    if explicit:
        return explicit
    candidates = [
        os.path.join(HERE, pool_name),            # 技能根目录（当前 INST 布局）
        os.path.join(HERE, "watch", pool_name),   # 旧意图 / DEV 可能的子目录
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
...
def main() -> int:
    ...
    pool_path = _resolve_pool_path(pool_name, args.pool)
    if not os.path.exists(pool_path):
        raise SystemExit(
            "找不到本地池：%s（可加 --pool 指定；已搜索技能根与 watch/ 子目录）" % pool_path
        )
