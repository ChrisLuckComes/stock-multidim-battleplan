...
def test_hard_stop_name_matches_value():
    ...
    for label, k in cases:
        ...
        sp = stop_plan(bars, "platform_break", z, atr_v)
        assert sp["hard_anchor"] != "突破位", (label, sp)
...
def test_hard_stop_widens_out_of_noise_band():
    ...
    sp = stop_plan(bars, "platform_break", z, atr_v)
    assert sp["hard_anchor"] != "突破位", sp
    assert sp["hard"] < 107.57, sp
...
def test_ash_late_is_not_a_ban():
    ...
    """
    src = (Path(__file__).resolve().parents[2] / "probe_intraday.py").read_text(
        encoding="utf-8")
...
def test_band_pos_is_signal_time_not_full_day():
    ...
    src = (Path(__file__).resolve().parents[2] / "research" / "backtest_intraday.py").read_text(
        encoding="utf-8")
